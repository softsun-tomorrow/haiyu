...
//            $config = get_config();
//            $cache = array_merge($cache, $config);
//            ksort($cache);
			$this->CI->cache->save($cachestr,$cache,2592000);
		}
		return $cache;
	...
	function loadProductSearch($model,$ismult=true){
		$cachestr = 'model_search';
		$str = '<form name=search method="post" action="'.site_url('search'.$this->langurl).'">';
		if($ismult){
			$str .= '<select name="model" class="sselect">';
		...
		$str .= '<input type="text" class="searchtxt" value="'.lang('searchval').'" name="keyword" onclick="if(this.value==\''.lang('searchval').'\') this.value=\'\'" />
 				<input class="searchbtn" type="submit" value="'.lang('search').'">
			</form>';
		return $str;
	}
	...
	function loadSlideType($position) {
		$cachestr = 'slideType_'.$this->currentLang.'_single_'.$position;
		$cache = $this->CI->cache->get($cachestr);
		if(!$cache){
			$cache = $this->CI->Data_model->getSingle(array('position'=>$position,'lang'=>$this->currentLang,'status'=>1),'type');
	...
	function loadCategoryById($id){
		$cachestr = 'category_'.$this->currentLang.'_singleById_'.$id;
		$cache = $this->CI->cache->get($cachestr);
		if(!$cache){
			$cache = $this->CI->Data_model->getSingle(array('lang'=>$this->currentLang,'id'=>$id),'category');
			$this->CI->cache->save($cachestr,$cache,2592000);
		}
		return $cache;
	}
	...
	function loadThisCategory($category){
		...
		$cache = $this->CI->cache->get($cachestr);
		if(!$cache){
			$data = $this->CI->Data_model->getData(array('isdisabled'=>0,'isdisplay'=>1,'lang'=>$this->currentLang,'lft >'=>$category['top']['lft'],'lft <'=>$category['top']['rht']),'listorder,lft',0,0,'category');
			$cache = $this->handleCategoryData($data);
			$this->CI->cache->save($cachestr,$cache,2592000);
		}
		return $cache;
	...
	function loadCdata($category){
		$cachestr = 'category_'.$this->currentLang.'_left_'.$category['top']['id'];
		$cache = $this->CI->cache->get($cachestr);
		if(!$cache){
			$data = $this->CI->Data_model->getData(array('lang'=>$this->currentLang),'order_id',0,0,'customise');
			$cache = $this->handleCategoryData($data);
			$this->CI->cache->save($cachestr,$cache,2592000);
		}
		return $cache;
	...
	function loadAttributeByID($id) {
		$cachestr = 'attribute'.$this->currentLang.'_singleById_'.$id;
		$cache = $this->CI->cache->get($cachestr);
		if(!$cache){
			$cache = $this->CI->Data_model->getSingle(array('lang'=>$this->currentLang,'status'=>1,'id'=>$id),'attribute');
			$this->CI->cache->save($cachestr,$cache,2592000);
		}
		return $cache?$cache:array();
	}
	
	function loadAttributeByParent($parent) {
		$cachestr = 'attribute'.$this->currentLang.'_singleByParent_'.$parent;
		$cache = $this->CI->cache->get($cachestr);
		if(!$cache){
			$cache = $this->CI->Data_model->getData(array('lang'=>$this->currentLang,'status'=>1,'parent'=>$parent),'listorder',0,0,'attribute');
			$this->CI->cache->save($cachestr,$cache,2592000);
		}
		return $cache?$cache:array();
	}
	
	function loadAttributeByName($name){
		$cachestr = 'attribute'.$this->currentLang.'_singleByName_'.$name;
		$cache = $this->CI->cache->get($cachestr);
		if(!$cache){
			$cache = $this->CI->Data_model->getSingle(array('lang'=>$this->currentLang,'status'=>1,'name'=>$name),'attribute');
			$this->CI->cache->save($cachestr,$cache,2592000);
		}
		//$cache = $this->CI->Data_model->getSingle(array('lang'=>'zh_cn','status'=>1,'name'=>$name),'attribute');
		
		return $cache?$cache:array();		
	}
	
	function loadThisAttribute($ids) {
		$cachestr = 'attribute'.$this->currentLang.'_singleById_'.$ids;
		$cache = $this->CI->cache->get($cachestr);
		if(!$cache){
			$idsarr = explode(',',$ids);
			$findinsetarr = array();
			...
			foreach($idsarr as $id){
				$findinsetarr[] = " FIND_IN_SET(".$id.",id)";
			}
			$findinsetstr = '('.implode(' OR ',$findinsetarr).')';
			$this->CI->db->where('lang',$this->defaultLang);
			...
			$this->CI->db->where($findinsetstr,NULL,FALSE);
			$this->db->order_by('id desc');
			$data = $this->CI->db->get('attribute')->result_array();
			$cache = $data;
			//$cache = $this->CI->Data_model->getSingle(array('lang'=>$this->currentLang, 'status'=>1),'attribute');
			$this->CI->cache->save($cachestr,$cache,2592000);
		}
		return $cache?$cache:array();
	}
	
	function loadAllAttribute() {
		$cachestr = 'attribute'.$this->currentLang.'_all';
		$cache = $this->CI->cache->get($cachestr);
		if(!$cache){
			$cache = $this->CI->Data_model->getData(array('lang'=>$this->currentLang,'status'=>1),'listorder,id',0,0,'attribute');
			$this->CI->cache->save($cachestr,$cache,2592000);
		}
		return $cache?$cache:array();
	}	
	...
	function handleCategoryData($data){
		$list = array();
		foreach($data as $item){
			$item['url'] = (isset($item['isexternal']) && $item['isexternal']==1)?$item['externalurl']:site_url('category/'.$item['dir'].$this->langurl);
			$item['rssurl'] = (isset($item['isexternal']) && $item['isexternal']==1)?$item['externalurl']:site_url('rss/'.$item['dir'].$this->langurl);
			//$item['url'] = $item['isexternal']==1?$item['externalurl']:site_url('category/'.$item['dir']).$this->langurl;
			//$item['rssurl'] =  $item['isexternal']==1?$item['externalurl']:site_purl('rss/'.$item['dir']).$this->langurl;
			$item['thumb'] = get_image_url($item['thumb']);
			$item['color'] = (isset($item['color']) && $item['color']=='')?'':' color:'.$item['color'].'; ';
	...
	function handleModelData($data){
		...
		foreach($data as $item){
			if(isset($item['id'])){
				$item['categoryid'] = isset($category[$item['category']]['id']) ? $category[$item['category']]['id'] : '';
				$item['categoryname'] = isset($category[$item['category']]['name']) ? $category[$item['category']]['name'] : '';
				$item['categoryurl'] = isset($category[$item['category']]['url']) ? $category[$item['category']]['url'] : '';
				$item['categorymodel'] = isset($category[$item['category']]['model']) ? $category[$item['category']]['model'] : '';
				$item['url'] = isset($category[$item['category']]['dir']) ? site_url('detail/'.$category[$item['category']]['dir'].'/'.$item['id'].$this->langurl) : '';
				$item['thumb'] = get_image_url($item['thumb']);
				$item['color'] = $item['color']==''?'':'color:'.$item['color'].';';
