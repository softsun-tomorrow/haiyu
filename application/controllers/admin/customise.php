...
class Customise extends CI_Controller {
	var $tablefunc = 'customise';
	var $fields = array('title','spic','bpic','content','type','disable','order_id','create_time','download_url','pdf','info1','info2');
	...
	public function index(){
		...
		$data = $this->Data_model->getData($getwhere,'order_id,id desc',$pagearr['pagenum'],($pagearr['currentpage']-1)*$pagearr['pagenum']);
		$res = array(
				'tpl'=>'list',
	...
	function _setlist($data,$ismultiple=true){
		...
		foreach($newdata as $key=>$item){
			...
			$categorystr = isset($this->categoryarr[82])?'[<a href="'.site_url('category/'.$this->categoryarr[82]['dir']).$this->langurl.'" target="_blank"><font color="green">'.$this->categoryarr[82]['name'].'</font></a>]':'';
			$newstr.='<tr id="tid_'.$item['id'].'">
			<td width=30><input type=checkbox name="optid[]" value='.$item['id'].'></td>
			<td width=50><input type="hidden" name="ids[]" value="'.$item['id'].'"><input type="text" name="listorder[]" class="input-order" size="3" value="'.$item['order_id'].'"></td>
			<td width=40>'.$item['id'].'</td>
			<td>'.$categorystr.$item['title'].'</a></td>
			<td width=50 >'.lang('status'.$item['disable']).'</td>
