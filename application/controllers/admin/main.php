...
class Main extends CI_Controller {
	...
	public function attrlist($uploaddir = 'attachment'){
		$usergroupid=$this->session->userdata('usergroup');
		if(!$usergroupid){
			$result = array('error'=>1,'message'=>lang('nopur'));
			echo json_encode($result);exit;
		}
		$attr_url = 'data/'.$uploaddir.'/';
	...
	function attrupload($uploaddir = 'attachment'){
		$usergroupid=$this->session->userdata('usergroup');
		if(!$usergroupid){
			$result = array('error'=>1,'message'=>lang('nopur'));
			echo json_encode($result);exit;
		}
		$save_path = 'data/'.$uploaddir.'/';
		$ext_arr = array(
	...
	function attrproductsupload($uploaddir = 'products'){
		$usergroupid=$this->session->userdata('usergroup');
		if(!$usergroupid){
			$result = array('error'=>1,'message'=>lang('nopur'));
			echo json_encode($result);exit;
