...
class AjaxProduct extends CI_Controller {
	var $defaultLang;
	...
	public function index(){	
		...
		if('product' == $thiscategory['model']){
            if('gear' == $thiscategory['dir']) {
                if ($this->uri->segment(5) && 'g' != $this->uri->segment(5)) {
                    $genderString = $this->uri->segment(5);
                    $genderArray = explode('_', str_replace('g_', '', $genderString));
                    foreach ($genderArray as $val) {
                        $where[] = $val;
                    }
                }
                if ($this->uri->segment(6) && 'c' != $this->uri->segment(6)) {
                    $categoryString = $this->uri->segment(6);
		...
		$currentpage = ($this->uri->segment(7)&&is_numeric($this->uri->segment(7)))?intval($this->uri->segment(7)):intval($this->uri->segment(3));
