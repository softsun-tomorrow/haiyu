...
var WaterFull = {
    ...
    createChild:function(thumb,title,puttime,url,description){
        var str = '<div class="pic"><img src="'+thumb+'" alt="" /></div>'+
					'<div class="text">'+
                    	'<div class="text-content">'+
                            '<strong>'+title+'</strong>'+
                            '<p class="date">'+puttime+'</p>'+
                            '<p>'+description+'</p>'+
						'</div>'+
                        '<div class="read-more"><a href="'+url+'">'+read_more+'</a></div>'+
                    '</div>';
	...
	// water execute
			...
			$.get(ajaxTo,function(data){
				...
				var i = 0,rows = WaterFull.getRowByHeight(),div,k;
				if(dataTmp.isEmpty == 1) {
					...
					for(;dataTmp[i];i++){
						div = WaterFull.createChild(dataTmp[i].thumb, dataTmp[i].title,dataTmp[i].puttime,dataTmp[i].url,dataTmp[i].description);
